...
for i_con_a = 1:numel(unique(t.attcon))
    for i_con_t = 1:numel(unique(conmat.mats.task))
        % index the condition number that fullfills the attention tracking condition t.uniquecon
        t.idx = t.attcon==i_con_a & conmat.mats.task == i_con_t;
...
% [1, 2, 3] [target class 1, target class 2, distractor(other uncued stimulus)]
...
for i_task = 1:2 % RDK vs square
    ...
    conmat.mats.eventdiscrtype(t.idx)=3;
end
...
for i_class = 1:2 % for target event classes
    t.idx = conmat.mats.eventdiscrtype == i_class & conmat.mats.task == 2;
